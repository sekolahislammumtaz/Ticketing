...
export async function exportTicketsToWord(eventInfo, participants, selectedDivisionName = '') {
  if (!participants || participants.length === 0) {
    alert('Tidak ada data peserta untuk di-ekspor!');
    return;
  }

  const divisionSuffix = selectedDivisionName ? ` - DIVISI ${selectedDivisionName.toUpperCase()}` : '';
  ...
  const headerParagraphs = [
    new Paragraph({
      children: [
        new TextRun({
          text: `DAFTAR TIKET PESERTA - ${eventInfo?.name?.toUpperCase() || 'ACARA'}${divisionSuffix}`,
          bold: true,
          size: 28,
          color: '0369A1',
        }),
  ...
  const divTag = selectedDivisionName ? `_Divisi_${selectedDivisionName.replace(/\s+/g, '_')}` : '';
  const fileName = `Tiket_${(eventInfo?.name || 'Acara').replace(/\s+/g, '_')}${divTag}.docx`;
